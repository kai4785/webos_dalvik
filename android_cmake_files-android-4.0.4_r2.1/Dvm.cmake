...
# Compiler defines.
#
concat(LOCAL_CFLAGS -fstrict-aliasing -Wstrict-aliasing=2 -fno-align-jumps)
concat(LOCAL_CFLAGS -Wall -Wextra -Wno-unused-parameter)
concat(LOCAL_CFLAGS '-DARCH_VARIANT="${dvm_arch_variant}"' )
...
set(dvm_make_debug_vm false)
if(NOT "${DEBUG_DALVIK_VM}" STREQUAL "")
  set(dvm_make_debug_vm ${DEBUG_DALVIK_VM})
endif()

if(${dvm_make_debug_vm})
  #
  # "Debug" profile:
  # - debugger enabled
  # - profiling enabled
  ...
  # - assert()
  #
  concat(LOCAL_CFLAGS -DWITH_INSTR_CHECKS)
  #concat(LOCAL_CFLAGS -DWITH_EXTRA_OBJECT_VALIDATION) # Doesn't compile
  concat(LOCAL_CFLAGS -DWITH_TRACKREF_CHECKS)
  concat(LOCAL_CFLAGS -DWITH_EXTRA_GC_CHECKS=1)
  #concat(LOCAL_CFLAGS -DCHECK_MUTEX)
  concat(LOCAL_CFLAGS -DDVM_SHOW_EXCEPTION=3)
  # add some extra stuff to make it easier to examine with GDB
  concat(LOCAL_CFLAGS -DEASY_GDB)
  ...
  #
  #concat(LOCAL_CFLAGS -DNDEBUG -DLOG_NDEBUG=1)
  # "-O2" is redundant for device (release) but useful for sim (debug)
  #concat(LOCAL_CFLAGS -O2 -Winline)
  #concat(LOCAL_CFLAGS -DWITH_EXTRA_OBJECT_VALIDATION)
  concat(LOCAL_CFLAGS -DDVM_SHOW_EXCEPTION=1)
...
# TODO: Do we need an os/webos.cpp ??
#ifneq ($(filter arm mips,$(dvm_arch)),)
#  LOCAL_SRC_FILES += os/android.cpp
#else
  concat(LOCAL_SRC_FILES os/linux.cpp)
#endif

# TODO: Seems redundant for CMake
#set(WITH_COPYING_GC ${WITH_COPYING_GC})

if(${WITH_COPYING_GC})
  concat(LOCAL_CFLAGS -DWITH_COPYING_GC)
...
if(${dvm_arch} STREQUAL arm)
  #set(dvm_arch_variant armv7-a)
  #concat(LOCAL_CFLAGS -march=armv7-a -mfloat-abi=softfp -mfpu=vfp)
  #concat(LOCAL_CFLAGS -Werror)
  set(MTERP_ARCH_KNOWN true)
